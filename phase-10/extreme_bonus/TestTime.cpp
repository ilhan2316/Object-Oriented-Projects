...
int main(int argc, char* argv[]) {
    ...
    // -------------------------------------------------------------------------
    vector <<= 1;   // Addition with integer
    if(fails(Time{1, 2, 3} + 10, "01:02:13")) result |= vector;
    if(fails(10 + Time{1, 2, 3}, "01:02:13")) result |= vector;
    ...
    // -------------------------------------------------------------------------
    int testVector = 1;   // Addition with integer (new functionality)
    if (fails(Time{1, 2, 3} + 10, "01:02:13")) {
    result |= testVector;
    }
    if (fails(10 + Time{1, 2, 3}, "01:02:13")) {
    result |= testVector;
    }
    // -------------------------------------------------------------------------
  
    int testVector2 = 1;   // Additional test vector

    int seconds = 600;
    Time testTime{9, 30, 0};

    Time result1 = testTime + seconds;
    Time result2 = seconds + testTime;

    if (fails(result1, "09:40:00")) {
    result |= testVector2;
    }

    if (fails(result2, "09:40:00")) {
    result |= testVector2;
    }
    
    //one more test
    int additionalSeconds = 300;  // 5 minutes
    Time result3 = testTime + additionalSeconds;

    if (fails(result3, "09:35:00")) {
    result |= testVector2;
    }

    // Test subscript operator []
     Time testTime2(9, 30, 15);
    if (testTime2[0] != 9 || testTime2[1] != 30 || testTime2[2] != 15) {
        std::cerr << "FAIL: Subscript operator [] failed." << std::endl;
        result |= 1;
    }
